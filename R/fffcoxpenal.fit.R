...
#' @import grpreg
fcoxpenal.fit1 <- function(x, y, strata, offset, init, control,
                           weights, method,
                           pcols, pattr, assign, npcols, tuning.method, sm, alpha, theta, lambda, nlambda = NULL,
  ...
  if(penalty == "alasso") {penalty.f[penalty.where] <- 1/abs(init)
  }else {
    penalty.f[penalty.where] <-1# B
  }


  if(is.null(lambda)) {
    nlambda <- ifelse(is.null(nlambda), 20, nlambda)
    p.lambda <- glmnet(xx, y, family="cox",  nlambda=nlambda, standardize=FALSE, thresh=1, penalty.factor = penalty.f)$lambda
  }else {
    p.lambda <- lambda
  }
...
#  }
  ...
  for(i in 1:m){
    if(is.null(sm[[i]]$D)) {
      ...
      D[[i]] <- sm[[i]]$D/4}
    #   dmat<- diag(length(pcols[[i]]))
     #D[[i]] <- apply(dmat, 2, diff, 1, 2)/4
  ...
  for(i in 1:nlambda){
    ...
    p.fit1 <- ncpen1(Ystar, Vstar, family = "gaussian", penalty=ifelse(penalty=="alasso", "lasso", penalty),  lambda = p.lambda[i]*n/nystar, x.standardize = FALSE,  intercept=FALSE,   w.lambda = penalty.f)
    newbeta <- p.fit1$beta

    error <- max(abs(newbeta - oldbeta))
  ...
  if(cv.fit) coef[, (iter-1)*nlambda + i] <- fit.beta[,i]
  else{
  ...
  #lambda <- p.lambda[i]*n/nystar
    ...
    if (andersen) {coxfit <- .Call(survival:::Cagfit4,
                                 y, newxx, cox.newstrat, weights,
                                 offset,
                                 as.double(beta.nonzero),
                                 sort.start, sort.end,
                                 as.integer(method=="efron"),
                                 as.integer(0),
                                 as.double(control$eps),
