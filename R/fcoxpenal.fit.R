...
#iter <- (outer1-1)*control$outer.max2 + outer2
  for(outer1 in 1:nlambda){
    thetasave <- cbind(unlist(thetalist))
    for(outer2 in 1:control$outer.max){
  ...
  ## Calculate df
   ...
   if(done) break
      ...
      init <-   coef
      finit <-  coxfit$fcoef
      thetasave <- cbind(thetasave,  as.vector(unlist(thetalist)))
    } else{
    ...
    if (outer1==1) {
     ...
     # if(done) iterlist[[i]]$history <- c(thetasave2, loglik1, df, loglik1 -   df, loglik1 - dfc, loglik1 - 2 * df,  loglik1 - log(n)*df,  -loglik1/(n*(1-df/n)^2))
      ...
      thetasave2 <- cbind(thetasave2, thetasave)

    }
    history <- rbind(history, cbind(lambdalist[[i]],  iterlist[[i]]$history))

   test <- sapply(1:length(cutoff[[i]]), function(i) all(coefsave[cutoff[[i]]:n.coef,]==0))

  if(all(test==1)) break
  if(outer1 == nlambda) break
     ...
     thetalist[[i]] <- temp$theta
     lambdalist[[i]] <- lambda[outer1+1]*XG$m[i]
     iterlist[[i]] <- temp
   }
 ...
 # final.history <- as.data.frame(final.history)
  ...
  names(history) <- c("lambda","theta", "loglik", "df", "aic", "aicc", "bic", "gcv")
  if(tuning.method == "AIC") which <- max((1:(outer1*control$outer.max))[history$aic==max(history$aic)])
  else if (tuning.method == "BIC") which <- max((1:(outer1*control$outer.max))[history$bic==max(history$bic)])
  else if (tuning.method == "GCV") which <- max((1:(outer1*control$outer.max))[history$gcv==min(history$gcv)])
  ...
  var0 <- hinv.full%*%(-I0[nonzero, nonzero])%*%hinv.full
